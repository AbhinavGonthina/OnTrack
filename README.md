...
- **No browser storage anywhere.** Auth state lives in React context only, so a hard refresh logs you out by design, not by bug. This was a deliberate constraint from the start, not a retrofit.
