...
A full-stack job application tracker built for SWE/CS job searches — an event-sourced status pipeline, SQL-driven funnel analytics with a Sankey visualization, and a Gemini-powered resume/job-description fit check, on top of a real JWT-authenticated CRUD backend.

**Live demo:** [on-track-sigma.vercel.app](https://on-track-sigma.vercel.app) — click "Try Demo" for a read-only tour with seeded data, or sign up for your own account.
...
- **Check your resume against a job description** — paste a JD, get a Gemini-backed fit score, missing keywords, and rewritten resume bullets. Results are cached by a hash of the resume+JD pair, so re-viewing a result never re-calls the AI or costs anything twice.
- **Try it with zero setup** — a public, read-only demo mode with 12 realistic seeded applications, no signup required.

## Why it's built this way

This project exists to demonstrate real backend engineering, not to be an AI wrapper. The AI feature (fit analysis) is one feature among several — the CRUD, the event-sourced status model, and the SQL analytics are meant to stand on their own in a technical interview:

- **Event-sourced status pipeline.** Status changes are appended as immutable `StatusEvent` rows, not overwritten in place — so the full history survives, funnel analytics can be recomputed at any time, and "how many applications ever reached OA" is an honest query instead of an assumption.
- **SQL over an ORM shortcut for analytics.** The Sankey computation needs to compare each status event to the previous one per application — a `LAG()` window function — which isn't expressible in JPQL. `StatsService` uses `NamedParameterJdbcTemplate` with raw SQL for exactly this, while the rest of the app uses Spring Data JPA where a repository is the right tool.
- **Abuse/cost protection is treated as a first-class feature, not an afterthought.** General API rate limiting (Bucket4j, 20 req/min per user/IP) is separate from a dedicated Gemini rate limiter (20 calls/day per user) that's only consulted on an actual cache miss — so free cached re-reads never count against the budget, but real AI calls are capped even if a user (or a script) tries to hammer the endpoint.
- **No browser storage anywhere.** Auth state lives in React context only — a hard refresh logs you out by design, not by bug. This was a deliberate constraint from the start, not a retrofit.
...
**AI** — Google Gemini (`gemini-flash-latest`) via Spring's `RestClient`, structured JSON output, SHA-256 input-hash result caching.
...
**Backend** (needs a Postgres database — Neon's free tier works well):
```bash
cd backend
