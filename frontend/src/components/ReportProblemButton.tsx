...
export function ReportProblemButton({ variant = "icon" }: Props) {
  ...
  return (
    <>
      ...
      {isOpen &&
        createPortal(
          // Portaled to <body> rather than rendered in place: the nav bar's backdrop-blur
          // establishes a new containing block for position:fixed descendants, which was
          // pinning this modal to the nav (near the top of the page) instead of centering
          // it in the viewport when opened from the nav icon.
          <div
